...
import numpy as np
# User-defined module
import libs.MNC.tools._init_paths
#import _init_paths
import caffe
...
CLASSES = ('aeroplane', 'bicycle', 'bird', 'boat',
		   'bottle', 'bus', 'car', 'cat', 'chair','person')
#CLASSES = ('car')
...
def parse_args():
	...
	parser.add_argument('--def', dest='prototxt',
						...
						default='./libs/MNC/models/VGG16/mnc_5stage/test.prototxt', type=str)
	parser.add_argument('--net', dest='caffemodel',
						help='model to test',
						default='./libs/MNC/data/mnc_model/mnc_model.caffemodel.h5', type=str)
...
if __name__ == '__main__':
	...
	#im_names = ['0089.jpg']
	#demo_dir = '/home/minh2/MinhData/Car/data/CarCrash/Cleaned/0/images/'#./data/demo'
	#im_names = ['2008_000533.jpg', '2008_000910.jpg', '2008_001602.jpg',
	#	'2008_001717.jpg', '2008_008093.jpg']
	#demo_dir = './libs/MNC/data/demo'

	for i in range(27):
		if i <0:
			continue
		try:
			main_dir = '/home/minh2/MinhData/Car/data/CarCrash/Cleaned2/' 
			main_dir = '/home/minh2/MinhData/Fifth/' 
			main_dir = '/home/dinesh/CarCrash/data/Fifth/'
			demo_dir = main_dir + str(i) +'/'
			print(demo_dir)
			#demo_dir = '/home/minh2/MinhData/Car/data/CarCrash/yeshasvi/'
			#im_names = [f for f in os.listdir(demo_dir+ '/images/') if f.endswith('.png')]
			im_names = [f for f in os.listdir(demo_dir) if f.endswith('.png')]
			#im_names = [f for f in os.listdir(demo_dir+ '/images/') if f.endswith('.jpg')]
			im_names.sort()
			print(im_names)
			for im_name in im_names:
				...
				#gt_image = os.path.join(demo_dir,'images', im_name)
				#print(gt_image)
				im_o = cv2.imread(gt_image)
				#print(im_o)
				im = cv2.resize(im_o,(im_o.shape[1]/2,im_o.shape[0]/2))
				start = time.time()
			...
			#print(result_box, result_mask)
				pred_dict = get_vis_dict(result_box, result_mask, demo_dir + im_name, CLASSES)
				...
				if not os.path.exists(os.path.join(demo_dir, 'mask')):
					os.makedirs(os.path.join(demo_dir, 'mask'))
				target_cls_file = os.path.join(demo_dir, 'mask',im_name)
				cls_out_img = np.zeros((img_height, img_width, 3))
				for i in xrange(img_height):
					for j in xrange(img_width):
						cls_out_img[i][j] = color_map[inst_img[i][j]][::-1]
				#print(target_cls_file)
				#print(cls_out_img)
				cls_out_img = cv2.resize(cls_out_img,(im_o.shape[1],im_o.shape[0]))
				#cv2.imwrite('check.png', cls_out_img)
				print(target_cls_file)
				cv2.imwrite(target_cls_file, cls_out_img)
				classes = pred_dict['cls_name']
				for i in xrange(len(classes)):
				#	score = pred_dict['boxes'][i][-1]
					bbox = pred_dict['boxes'][i][:4]*2
					bbox_final = [bbox[0],bbox[1],bbox[2]-bbox[0],bbox[3]-bbox[1]]
					save = gt_image + ' ' + str(bbox_final[0]) + ' ' +  str(bbox_final[1]) + ' ' + str(bbox_final[2]) + ' ' + str(bbox_final[3]) 
					f = open(main_dir + 'boundingbox.txt',"a")
					f.write(save)
					f.write('\n')
					f.close()
					print(save)
