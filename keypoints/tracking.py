...
for main_loop in range(1,21):
    ...
    for index,name in enumerate(filenames):
        ...
        if index ==0:
            write_file(name.replace('/keypoints_txt/','/keypoints_tracked/'),data)
        ...
        filename = filename.replace('/keypoints_txt/','/keypoints_tracked/')
        print(filename)
        ...
        for ind,kp in enumerate(data):
            ...
            for ind_new,kp_new in enumerate(data_prev):
                BOXA = [int(l) for l in kp_new[1:5]]
                BOXB = [int(l) for l in kp[1:5]]
